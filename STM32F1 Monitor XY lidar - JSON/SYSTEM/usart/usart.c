...
u32 Active_Distance = 0; //解决Error: L6218E: Undefined symbol Active_Distance (referred from main.o).
...
{ 
	...
	if(USART_GetITStatus(USART2,USART_IT_RXNE)!=RESET) //串口2接收中断
	{
		...
		{  
			...
			if(count==400) //接收到一帧数据即400个字节
			{
				check_sum = verifyCheckSum(USART2_RX_BUF,400);
				if(check_sum == 1)  //判断校验和是否满足即数据的有效性
				{  
					//printf("get");
					TofM_Unpack_Data(USART2_RX_BUF);
				}
				count = 0;
				flag = 0;
			}		
